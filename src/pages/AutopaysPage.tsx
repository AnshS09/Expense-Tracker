    ...
    <div className="space-y-6 pb-12">
          ...
          <div key={item.id} className="bg-white dark:bg-[#151c28] p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              <div className="flex justify-between items-start">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider bg-surface-container-high dark:bg-slate-700 text-on-surface-variant dark:text-slate-300 px-2.5 py-1 rounded-full">
                  {item.frequency}
                </span>
              </div>

              <div>
                <h3 className="text-base font-bold text-on-surface dark:text-slate-100">{item.name}</h3>
                <p className="text-xs text-on-surface-variant dark:text-slate-400">
                  {getCategoryName(item.categoryId)} • {getWalletName(item.walletId)}
                </p>
              </div>

              <div className="pt-2 border-t border-outline-variant/60 dark:border-slate-700/60">
                ...
                <span className="text-xs font-semibold text-on-surface-variant dark:text-slate-400 block mt-1">
                  Next Due Date: <strong className="text-on-surface dark:text-slate-200">{item.nextDueDate}</strong>
                </span>
              </div>
            </div>

            <div className="pt-4 border-t border-outline-variant/60 dark:border-slate-700/60">
              <button
                onClick={() => setSelectedAutopay(item)}
                className="w-full bg-primary dark:bg-slate-100 text-on-primary dark:text-slate-900 py-2.5 rounded-xl font-bold text-xs hover:bg-slate-800 dark:hover:bg-slate-300 transition-colors shadow-sm"
              >
                Confirm Payment Now
              </button>
            </div>
      ...
      <Modal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Create Recurring Autopay">
        <form onSubmit={handleAddSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-on-surface-variant dark:text-slate-400 uppercase mb-1">Autopay Name</label>
            <input
              type="text"
              required
              value={name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-on-surface-variant dark:text-slate-400 uppercase mb-1">Amount</label>
              <input
                type="number"
                step="0.01"
                required
            ...
            <div>
              <label className="block text-xs font-semibold text-on-surface-variant dark:text-slate-400 uppercase mb-1">Frequency</label>
              <select
                value={frequency}
                onChange={(e) => setFrequency(e.target.value as AutopayFrequency)}
                className="w-full bg-surface-container-lowest dark:bg-slate-900 border border-outline-variant dark:border-slate-700/60 rounded-xl py-2.5 px-3 text-sm text-on-surface dark:text-slate-100 focus:ring-2 focus:ring-primary focus:outline-none"
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-on-surface-variant dark:text-slate-400 uppercase mb-1">Category</label>
              <select
                value={categoryId}
            ...
            <div>
              <label className="block text-xs font-semibold text-on-surface-variant dark:text-slate-400 uppercase mb-1">Primary Wallet</label>
              <select
                value={walletId}
          ...
          <div>
            <label className="block text-xs font-semibold text-on-surface-variant dark:text-slate-400 uppercase mb-1">First Due Date</label>
            <input
              type="date"
              required
              value={nextDueDate}
